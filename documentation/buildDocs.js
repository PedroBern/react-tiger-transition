...
var fs = require('fs');
var generateMarkdown = require('./generateComponentsMarkdown');
...
function buildDocs(api) {
  ...
  for (var filepath in api) {
    if (api[filepath].description.indexOf('@docIgnore') === -1){
      ...
      fs.writeFileSync('documentation/assets/' + name + '.md', markdown);
      process.stdout.write(filepath + ' -> ' + 'documentation/assets/' + name + '.md\n');
    }
  }
}
